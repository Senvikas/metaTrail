...
import dotDataCA from "./ConfigureCADots";
...
const Fip = () => {
  ...
  const { isAnimating, currentDotIndex, startAnimation, stopAnimation } =
    useDotAnimation(dotDataCA.length); // Use animation hook

  // Automatically open/close modals during animation
  useEffect(() => {
    if (isAnimating && currentDotIndex !== null) {
      setActiveStep(dotDataCA[currentDotIndex]); // Open modal for current dot
    } else {
