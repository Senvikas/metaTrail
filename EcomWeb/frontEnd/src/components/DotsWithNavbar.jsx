...
import dotData from "./ConfigureDots";
...
const Fip = () => {
  ...
  const { isAnimating, currentDotIndex, startAnimation, stopAnimation } =
    useDotAnimation(dotData.length); // Use animation hook

  // Automatically open/close modals during animation
  useEffect(() => {
    if (isAnimating && currentDotIndex !== null) {
      setActiveStep(dotData[currentDotIndex]); // Open modal for current dot
    } else {
