...
import dotDataFop from "./ConfigureFopDots";
...
const Fip = () => {
  ...
  const { isAnimating, currentDotIndex, startAnimation, stopAnimation } =
    useDotAnimation(dotDataFop.length); // Use animation hook

  // Automatically open/close modals during animation
  useEffect(() => {
    if (isAnimating && currentDotIndex !== null) {
      setActiveStep(dotDataFop[currentDotIndex]); // Open modal for current dot
    } else {
